...
def openDisplaydir(self):
    ...
    file = tk.filedialog.askopenfile(filetypes=[("csv files", "*.csv")])
...
def loadData(self):
    ...
    try:
        ...
        if self.ignoreFirstLineCmd.get() == 0:

            for line in data:
                if len(line) > 1:

                    xVal, yVal, eVal = line.split(',')

                    x.append(float(xVal))
                    y.append(float(yVal))
                    e.append(float(eVal))

                    preview = ('%s, %s, %s' % (xVal, yVal, eVal))
                    self.filePreview.insert(i, preview)
                    i += 1

        elif self.ignoreFirstLineCmd.get() == 1:
        ...
        if filename != 0:
            ...
            tk.messagebox.showinfo('Error', (
                    ...
                    '%s\n'
                    '\n'
                    'The data should be a comma separated value file containing '
...
def plotannotate(self):
    ...
    m = self.marker_option.get()
    l = self.line_option.get()

    plt.errorbar(x, y, e, linestyle='none', color='k', marker=m)
    [xmin, xmax]=plt.gca().get_xlim()
    xlim = np.linspace(xmin, xmax, 500)

    if redchisqrd <= chisqrdlimit[0] and redchisqrd >= chisqrdlimit[1]:
        plt.plot(xlim, np.polyval(p, xlim), 'b', linestyle=l)
    else:
        plt.plot(xlim, np.polyval(p, xlim), 'r', linestyle=l)
...
class MainMenu:
    def __init__(self,master):
        ...
        self.markerOption = ttk.OptionMenu(
            self.frame1, self.marker_option,
            '', 'None', '.', 'o', 'x', 'o', '*', '+'
        )
        self.markerOption.configure(width=5)

        self.lineStyleLabel = ttk.Label(self.frame1, text="Line style: ")
        self.lineOption = ttk.OptionMenu(
            self.frame1, self.line_option,
            '', '-', '--', '-.', ':', 'None'
        )
        ...
        self.ignoreFirstLineCheck = ttk.Checkbutton(
            self.frame2, variable=self.ignoreFirstLineCmd, onvalue=1, offvalue=0,
        ...
        self.ignoreFirstLineCheck.grid(row=2, column=1, sticky='W', padx=1, pady=1)

        self.loadData.grid(row=3, column=0, sticky='NESW', padx=5, pady=1, columnspan=2)
    ...
    def __init__(self, master):
        ...
        self.helpText.insert('1.0',
            (
                ...
                "Marker/Linestyles................................Select the marker and linestyle\n"
                "\n"
                ...
                "================================================================================\n"
                "\n"
                "This program only loads *.csv files which can be easily created and edited using\n"
                "Microsoft excel\n"
                "\n"
                "Load new data.............................................Loads a new data file\n"
                "Preview of data will be displayed in the preview window and current directory\n"
                "will be displayed\n"
                "Ignore first line........The first line of the select file will be ignored when\n"
